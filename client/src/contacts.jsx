...
export default function Contacts({setmsg,setfriend,hide,sethide,allmessage}){
        ...
        const clone={
           ...
           const [jet,setjet]=useState(true);
                ...
                <div  className={hide===true ?  "contacts max-[750px]:hidden" : "contacts" } >
                    ...
                    <div className="h-100vh">
                    {
                     ...
                     allcon.map((see)=>{
                        return(
                            <>
                             <div onClick={()=>{sethide(true);setmsg(true);setfriend(see.conuser)}} className="flex my-1 py-1 bg-slate-300">
                             <div className="relative w-12 h-12 flex items-center justify-center hover:bg-slate-400 ">
                             <Avatar theuser={see.conuser}  setjet={setjet}/>
                             </div>
                             <div>
                                ...
                                {see.mix.lastmsg.videourl &&
                                    <IoVideocam size={14}/>}{
                                        see.mix.lastmsg.text.length > 30 ?  <p className="ml-0.5  text-xs">{see.mix.lastmsg.text.substr(0,30)+"..."}</p> :
